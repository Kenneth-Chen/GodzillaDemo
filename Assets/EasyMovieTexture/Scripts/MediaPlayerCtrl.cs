...
public class MediaPlayerCtrl : MonoBehaviour {
	...
	void Update()
	{
		...
		if(m_CurrentState == MEDIAPLAYER_STATE.PLAYING)
		{
			...
			Debug.Log ("MEDIAPLAYER SEEKPOS " + Call_GetSeekPosition().ToString());
		}
		...
		{
			Debug.Log ("MEDIAPLAYER IS " + Call_GetStatus().ToString());
			...
			{
				Debug.Log ("MEDIAPLAYER IS ERROR " + Call_GetError() + "," + Call_GetErrorExtra());
	...
	private void Call_UpdateVideoTexture()
	{
		if(m_TargetMaterial)
		{
			if(m_TargetMaterial.renderer.material.mainTexture != m_VideoTexture)
			{
				m_TargetMaterial.renderer.material.mainTexture = m_VideoTexture;
			}
